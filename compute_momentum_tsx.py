...
def commit_log(text, path="compute_momentum_tsx_output.txt"):
    ...
    payload = {"message": "momentum tsx output", "content": content_b64}
...
def trading_day_price(prices_by_date, n):
    # n=5 -> prezzo di 5 giorni di CONTRATTAZIONE fa (convenzione Yahoo),
    # non "n giorni di calendario piu' vicino"
...
while True:
    r = requests.get(f"{SUPABASE_URL}/rest/v1/stocks", headers=headers_r,
        params={"select":"ticker","exchange":"eq.TSX","in_universe":"eq.true","limit":"1000","offset":str(offset)})
    batch = r.json()
    if not isinstance(batch,list) or not batch: break
    all_tickers.extend(s["ticker"] for s in batch)
    offset += 1000
    if len(batch) < 1000: break
log(f"TSX: {len(all_tickers)} titoli")
...
for i, ticker in enumerate(all_tickers):
    try:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/prices_eod", headers=headers_r,
            params={"select":"date,adj_close","ticker":f"eq.{ticker}","exchange":"eq.TSX",
        ...
        row = {"ticker": ticker, "exchange": "TSX", "price": latest_price,
