...
const clr = (v: number | null | undefined): string => { return '' }
          ...
          <tr>
            {COLUMNS.map((c, ci) => (
              <th key={c.key} onClick={() => toggle(c.key)}
                style={{
                  minWidth: c.width, userSelect: 'none',
                  cursor: (NO_SORT.has(c.key) || (!userId && LOCKED_GUEST.has(c.key))) ? 'default' : 'pointer',
                  ...(ci === 0 ? { position: 'sticky', left: 0, zIndex: 2, background: '#0d1017', boxShadow: '2px 0 4px rgba(0,0,0,0.3)' } : {})
                }}>
            ...
            <tr key={`${s.ticker}.${s.exchange}.${i}`}
              ...
              {COLUMNS.map((c, ci) => {
                ...
                return (
                  <td key={c.key} style={{
                    maxWidth: c.width,
                    ...(ci === 0 ? { position: 'sticky', left: 0, zIndex: 1, background: '#0d1017', boxShadow: '2px 0 4px rgba(0,0,0,0.3)' } : {})
                  }}>
                    {c.key === 'sector' && sectorColor ? (
                      <span className="truncate block text-[10px] font-600" style={{ color: sectorColor }}>{val}</span>
...
function Screener({ initExchange = 'MIL', initSector = 'All', initEpsMom = '', onSelectStock, userId = null, initValMin = 0, initGrowMin = 0, initCombinedMin = 0, showAll = false, maxRows: maxRowsProp }: {
  ...
  const stocksWithEurCap = stocks.map(s => ({ ...s, mktCap: s.mktCap ?? null }))
    ...
    <div className="max-w-2xl space-y-5 fade-in">
      ...
      {tab === 'terms' && (
        <div className="space-y-4">
          {([
            ['1. Service Ownership', 'ForwardAlpha is developed and operated by Andrea Meschini, based in Verona, Italy. Contact: andrea@forwardalpha.pro.'],
            ['2. Service Description & Beta Phase', 'ForwardAlpha is a web-based quantitative financial screening and research tool offering proprietary Value Score and Growth Score models (percentile rankings 1–100). The Platform is currently in Beta. Registration includes a 14-day free trial with no credit card required.'],
            ['3. Legal Disclaimer – No Financial Advice', 'ForwardAlpha is for informational and educational purposes only. The Owner is not a registered financial advisor and does not provide investment advisory services. No content constitutes a personalised recommendation or offer to buy/sell financial instruments under MiFID II. Investing involves significant risk of capital loss. Past performance is not indicative of future results.'],
            ['4. Data Accuracy & Limitation of Liability', 'Market prices are delayed approximately 15–20 minutes. Fundamental data is updated end of day. The service is provided "as is" without warranty of accuracy, completeness or uninterrupted availability. The Owner shall not be liable for any direct or indirect losses arising from use of the Platform.'],
            ['5. Intellectual Property', 'All content (UI, source code, Value Score and Growth Score algorithms, brand name "ForwardAlpha") is the exclusive property of Andrea Meschini. Strictly prohibited: automated scraping or crawling, reverse engineering of algorithms, redistribution or resale of Platform data to third parties without prior written consent.'],
            ['6. Account, Suspension & Termination', 'Users are responsible for their login credentials and all account activity. The Owner reserves the right to suspend or terminate any account immediately and without notice in case of breach of these Terms, fraudulent use, or automated data extraction.'],
            ['7. Clausole Vessatorie (Art. 1341–1342 c.c.)', 'In accordance with Italian civil law, the following clauses are explicitly brought to your attention and require specific acceptance: (a) limitation of liability (clause 4); (b) exclusive jurisdiction of the Court of Verona (clause 8). By creating an account you confirm you have read and specifically accepted these clauses.'],
            ['8. Governing Law & Jurisdiction', `These Terms are governed by Italian law. Any disputes shall be subject to the exclusive jurisdiction of the Court of Verona, without prejudice to mandatory consumer protection rights in the user's country of residence.`],
      ...
      {tab === 'privacy' && (
        <div className="space-y-4">
          {([
            ['1. Data Controller', 'Data Controller: Andrea Meschini, Verona (VR), Italy. Contact: andrea@forwardalpha.pro.'],
            ['2. Data We Collect', 'We collect: full name, email address, country of residence, hashed password (via Supabase Auth). We also collect usage data solely to improve the service.'],
            ['3. Legal Basis for Processing', 'Personal data is processed on the basis of: (a) contract performance (Art. 6.1.b GDPR); (b) legitimate interest (Art. 6.1.f GDPR); (c) consent (Art. 6.1.a GDPR) for optional newsletter communications.'],
            ['4. Data Retention', 'Account data is retained for as long as your account is active. If you delete your account, personal data is permanently deleted within 30 days.'],
            ['5. Your Rights (GDPR)', 'You have the right to: access your data, rectify inaccurate data, request erasure, restrict processing, data portability, and object to processing. Contact: andrea@forwardalpha.pro.'],
            ['6. Sub-processors', 'We use: Supabase Inc. (database and authentication, EU servers); Vercel Inc. (hosting and CDN). All sub-processors are contractually bound to handle data in accordance with GDPR.'],
            ['7. International Transfers', 'Data is stored on Supabase EU-based servers. Where any transfer outside the EEA occurs, appropriate safeguards (Standard Contractual Clauses) are in place.'],
            ['8. Newsletter', 'If you opt in during registration, your email will be used to send product updates. You may unsubscribe at any time.'],
          ] as [string,string][]).map(([title, body]) => (
            <div key={title} className="bg-surface border border-border rounded-lg p-4">
              <h3 className="font-700 text-text text-sm mb-2">{title}</h3>
              <p className="text-xs text-sub leading-relaxed">{body}</p>
      ...
      {tab === 'cookie' && (
        <div className="space-y-4">
          {([
            ['1. What are Cookies?', 'Cookies are small text files stored on your device when you visit a website, retransmitted on subsequent visits.'],
            ['2. Cookies Used by ForwardAlpha', 'We use only strictly necessary technical cookies: (a) Authentication & Session; (b) Security; (c) Preferences. No advertising, profiling or third-party tracking cookies are used.'],
            ['3. Legal Basis', 'Processing via technical cookies is based on contract performance (Art. 6.1.b GDPR). Since only strictly necessary cookies are used, prior consent is not required.'],
            ['4. Managing Cookies', 'You can control or delete cookies via your browser settings. Disabling technical cookies will prevent login and impair core Platform functionality.'],
            ['5. Contact', 'For any privacy or cookie queries: andrea@forwardalpha.pro.'],
          ] as [string,string][]).map(([title, body]) => (
            <div key={title} className="bg-surface border border-border rounded-lg p-4">
              <h3 className="font-700 text-text text-sm mb-2">{title}</h3>
              <p className="text-xs text-sub leading-relaxed">{body}</p>
...
function CookieBanner() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-surface border-t border-border px-4 py-3 flex items-center justify-between gap-4 text-xs text-muted">
      ...
      <button onClick={() => { localStorage.setItem('cookie-ok','1'); setVisible(false) }}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-bg">
      ...
      `}>
        ...
        {user ? (
          <div className="p-3 border-t border-border">
            <div className="text-[10px] text-muted truncate mb-1">{user.email}</div>
            <button onClick={() => supabase.auth.signOut()}
              className="w-full flex items-center gap-2 px-3 py-2 rounded text-xs text-muted hover:text-text hover:bg-white/5 transition-colors">
              <LogOut size={12} /> Log Out
            </button>
            <div className="text-[9px] text-muted/50 mt-1">⚠️ Prices indicative — new data source coming soon</div>
            <div className="text-[9px] text-muted/50">⚠️ Prices indicative — new data source coming soon</div>
            <div className="text-[9px] text-muted/50">Fundamentals: daily</div>
          </div>
        ) : (
          <div className="p-3 border-t border-border">
            <button onClick={() => setShowAuth(true)} className="btn-primary w-full text-xs py-2">
              <LogIn size={12} className="inline mr-1" /> Sign In / Register
            </button>
          </div>
        )}
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* TOPBAR */}
        <header className="bg-surface border-b border-border px-4 h-11 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebar(s => !s)} className="md:hidden text-muted hover:text-text">
              <Menu size={20} />
            </button>
            <div className="font-700 text-base md:hidden" style={{ fontFamily: 'IBM Plex Sans Condensed' }}>
              FORWARD<span style={{ color: 'var(--orange)' }}>ALPHA</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="badge badge-beta text-[9px]">🧪 BETA</span>
            {user ? (
              <button onClick={() => supabase.auth.signOut()} className="text-muted hover:text-text p-1">
                <LogOut size={16} />
              </button>
            ) : (
              <button onClick={() => setShowAuth(true)} className="btn-primary text-xs px-3 py-1.5">
                Sign In
              </button>
            )}
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-3 md:p-4">
          ...
          {page === 'screener' && <Screener key={`screener-${scrExchange}-${scrSector}`} initExchange={scrExchange} initSector={scrSector} initEpsMom={scrEpsMom} onSelectStock={setDetailStock} userId={user?.id || null} />}
          ...
          {page === 'MIL'   && <Screener key="MIL"   initExchange="MIL"   onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'PA'    && <Screener key="PA"    initExchange="PA"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'XETRA' && <Screener key="XETRA" initExchange="XETRA" onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'LSE'   && <Screener key="LSE"   initExchange="LSE"   onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'OM'    && <Screener key="OM"    initExchange="OM"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'OB'    && <Screener key="OB"    initExchange="OB"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'SWX'   && <Screener key="SWX"   initExchange="SWX"   onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'MC'    && <Screener key="MC"    initExchange="MC"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'AS'    && <Screener key="AS"    initExchange="AS"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'HE'    && <Screener key="HE"    initExchange="HE"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'BR'    && <Screener key="BR"    initExchange="BR"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'GR'    && <Screener key="GR"    initExchange="GR"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'CPSE'  && <Screener key="CPSE"  initExchange="CPSE"  onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'VI'    && <Screener key="VI"    initExchange="VI"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'LS'    && <Screener key="LS"    initExchange="LS"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          {page === 'IR'    && <Screener key="IR"    initExchange="IR"    onSelectStock={setDetailStock} userId={user?.id || null} />}
          ...
          {page === 'bestideas' && (user
            ...
            : <LoginGate onLogin={() => setShowAuth(true)} title="Best Ideas Europe" />
          )}
          {page === 'bestvalue' && (user
            ...
            : <LoginGate onLogin={() => setShowAuth(true)} title="Best Value Europe" />
          )}
          {page === 'bestgrowth' && (user
            ...
            : <LoginGate onLogin={() => setShowAuth(true)} title="Best Growth Europe" />
          )}
          ...
          {page === 'portfolio' && (
            <div className="p-4">
              ...
              <p className="text-muted text-sm mt-2">Portfolio feature coming soon.</p>
            </div>
          )}
          {page === 'myscreen' && (
            <MyScreen userId={user?.id || null} onSelectStock={setDetailStock} />
          )}
        </main>
      </div>
