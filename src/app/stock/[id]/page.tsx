          ...
          return (
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)',
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                ...
                <a href={`https://news.google.com/search?q=${(stock as any).exchange === 'US' ? encodeURIComponent((stock as any).yahoo_ticker || ticker)+'+stock' : encodeURIComponent((stock as any).company || ticker)+'+'+encodeURIComponent((stock as any).yahoo_ticker || ticker)+((stock as any).isin ? '+'+encodeURIComponent((stock as any).isin) : '')}&hl=en&gl=US&ceid=US:en`} target="_blank" rel="noopener noreferrer" style={{ background:'#1a73e8', color:'#fff', fontFamily:'IBM Plex Sans Condensed', fontWeight:700, fontSize:12, padding:'7px 14px', borderRadius:3, textDecoration:'none', display:'inline-flex', alignItems:'center', gap:6 }}>📰 News ↗</a>
              </div>
            </div>
          )
        })()}
