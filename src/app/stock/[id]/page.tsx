...
export async function generateMetadata({ params }: { params: { id: string } }) {
  ...
  try {
    const [sRes, fRes] = await Promise.all([
     fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'https://forwardalpha.pro'}/api/db/stocks?ticker=${ticker}&exchange=${exchangeCode}`, { next: { revalidate: 3600 } }),
     fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'https://forwardalpha.pro'}/api/db/stocks?ticker=${ticker}&exchange=${exchangeCode}`, { next: { revalidate: 3600 } })
    ])
    const sData = await sRes.json()
    const stock = sData.stocks?.[0]
    ...
    return {
     title,
     description: desc,
     openGraph: { title, description: desc, url: `https://forwardalpha.pro/stock/${params.id}` },
    }
  } catch {
