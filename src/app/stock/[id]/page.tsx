          ...
          return (
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)',
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                ...
                {(stock as any).sector && <button onClick={() => { window.location.href = `/?page=${['TSE','SEHK','ASX'].includes((stock as any).exchange) ? 'asiapacific' : (stock as any).exchange === 'US' ? 'northamerica' : (stock as any).exchange === 'TSX' ? 'nascreen' : 'screener'}&sector=${encodeURIComponent((stock as any).sector)}` }} style={{ background:'var(--surface2)', color:'var(--text3)', fontFamily:'IBM Plex Sans Condensed', fontSize:11, fontWeight:700, padding:'6px 12px', borderRadius:3, border:'1px solid var(--border)', cursor:'pointer' }}>🏭 {(stock as any).sector}</button>}
