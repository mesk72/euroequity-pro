...
export default function NewsPage() {
  ...
  const generateReportBest = async () => {
    ...
    const fmtNewsItem = (n: NewsItem) => {
      ...
      if (n.valueScore != null) {
        line += ' | Val ' + Math.round(n.valueScore ?? 0) + ' Grw ' + Math.round(n.growthScore ?? 0) + ' Best ' + Math.round(n.bestScore ?? 0)
      }
      if (n.link) line += '\n  📰 Read article → ' + n.link
      if (n.ticker && n.exchange) {
        line += '\n  📊 View on ForwardAlpha → https://www.forwardalpha.pro/stock/' + n.ticker + '-' + n.exchange
      }
      return line
    }
  ...
  const generateReport = async () => {
    ...
    const fmtNewsItem = (n: NewsItem) => {
      ...
      if (n.ticker) line += '[' + n.ticker + '] '
      line += n.title
      if (n.valueScore != null) line += ' | Val ' + Math.round(n.valueScore ?? 0) + ' Grw ' + Math.round(n.growthScore ?? 0) + ' Best ' + Math.round(n.bestScore ?? 0)
      if (n.link) line += '\n  📰 Read article → ' + n.link
...
<hr>
${body}
<p class="footer">Generated by <a href="https://www.forwardalpha.pro">ForwardAlpha</a> — Global Equity Research</p>
</body>
</html>`
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {item.ticker && item.valueScore != null && (
                    <div style={{ display: 'flex', gap: 6, marginTop: 4, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#3b82f6' }}>Val {Math.round(item.valueScore ?? 0)}</span>
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#22c55e' }}>Grw {Math.round(item.growthScore ?? 0)}</span>
                      <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--orange)' }}>Best {Math.round(item.bestScore ?? 0)}</span>
                    </div>
                  )}
                </div>
