...
def commit_log(text, path="fix_combined_rank_eu_output.txt"):
    ...
    payload = {"message": "fix combined rank eu output", "content": content_b64}
...
# Legge value_score/growth_score gia' calcolati (corretti, per singola borsa)
# per TUTTA l'Europa insieme, poi ricalcola combined_rank sull'unione,
# come fa daily_us.py per US+TSX.
all_data = []
...
log(f"Titoli EU con value+growth score: {len(all_data)}")
