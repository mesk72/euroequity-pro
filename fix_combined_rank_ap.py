...
def commit_log(text, path="fix_combined_rank_ap_output.txt"):
    ...
    payload = {"message": "fix combined rank ap output", "content": content_b64}
...
# Best Score combinato per tutta l'Asia Pacific: Giappone, Hong Kong,
# Australia, Corea, Singapore insieme (confermato dall'utente, 5 mercati)
AP_COMBINED_EXCHANGES = ['TSE', 'SEHK', 'ASX', 'KRX', 'SGX']

all_data = []
for ex in AP_COMBINED_EXCHANGES:
    universe = set()
    offset = 0
    while True:
...
log(f"Titoli AP (JP+HK+AU) con value+growth score: {len(all_data)}")
