...
while True:
    ...
    for d in batch:
        if d["ticker"] in universe and d.get("mkt_cap") is not None:
            all_caps.append(d["mkt_cap"])  # valori in MILIONI di $
    offset += 1000
    if len(batch) < 1000: break
...
print(f"Top 500 - max: ${top500[0]/1000:,.1f}B, min: ${top500[-1]/1000:,.1f}B")
print(f"Mediana top 500: ${statistics.median(top500)/1000:,.2f}B")
print(f"Media top 500: ${statistics.mean(top500)/1000:,.2f}B")
