...
# Rimuovi da in_universe i titoli senza prezzi
if no_price:
    print(f"\nRimuovo in_universe per {len(no_price)} titoli senza prezzi:")
    for ticker in no_price:
        r2 = requests.patch(f"{SUPABASE_URL}/rest/v1/stocks",
            headers=headers_up,
            params={"ticker":f"eq.{ticker}","exchange":f"eq.{EXCHANGE}"},
            json={"in_universe": False})
        print(f"  {ticker}: in_universe=false HTTP {r2.status_code}")

print(f"\n=== DONE ===")
print(f"In universe finale: {ok} titoli con prezzi")
print(f"Esclusi per mancanza prezzi: {len(no_price)}")
