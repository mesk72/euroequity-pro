...
print()
print("%-14s %6s %9s %9s %6s  %s" % ("MERCATO","TOT","ALLINEATI","INDIETRO","%","ULTIMA SEDUTA"))
TOT=0;ALL=0
for nome,lista in G:
    t=0; d=[]
    for ex in lista:
        ...
        while True:
            ...
            if len(b)<1000: break
    c=Counter(d)
    sed=c.most_common(1)[0][0] if c else "-"
    a=sum(1 for x in d if x>=sed)
    TOT+=t; ALL+=a
    print("%-14s %6d %9d %9d %5.1f%%  %s" % (nome,t,a,t-a,a/t*100 if t else 0,sed))
print()
...
for x in r: print("  %-6s %s ok=%s" % (x["market"],x["created_at"][:19],x["prices_updated"]))
