...
def guess_sector(name):
    ...
    if any(x in n for x in ["pharma","bio","medic","health","sanit","gesundh","sante"]):
        return "Health Care"
    if any(x in n for x in ["tech","software","digital","data","cloud","semi","chips","siemens","sap","asm","asml"]):
        return "Technology"
    if any(x in n for x in ["auto","motor","volkswagen","bmw","mercedes","stellantis","ferrari","renault","peugeot"]):
        return "Consumer Discretionary"
    if any(x in n for x in ["food","beverage","luxury","retail","unilever","lvmh","nestl","ferrero"]):
        return "Consumer Staples"
    if any(x in n for x in ["energy","eni","total","repsol","shell","oil","gas","petro"]):
        return "Energy"
    if any(x in n for x in ["material","steel","mining","chemical","basf","linde","akzo","solvay","arcelor"]):
        return "Materials"
    if any(x in n for x in ["telecom","telekom","telefonica","orange","vodafone","tim ","bte"]):
        return "Communication"
    if any(x in n for x in ["utility","enel","endesa","rwe","engie","a2a","hera","iren"]):
        return "Utilities"
    if any(x in n for x in ["real estate","realty","immobil","unibail","coima","property"]):
        return "Real Estate"
    if any(x in n for x in ["airbus","safran","defense","rheinm","industrial"]):
        return "Industrials"
    return "Other"
...
def get_fundamentals(ticker_exchange):
    ...
    def sm(val, m):
        try:
            return float(val) * m if val is not None else None
        except Exception:
            return None

    def sd(val, d):
        try:
            return float(val) / d if val is not None and d else None
        except Exception:
            return None
...
def enrich_df(df, max_enrich=None):
    if df.empty:
        return df
    df = df.copy()
    top_idx = df.index if max_enrich is None else df.nlargest(min(max_enrich, len(df)), "Volume", keep="all").index
    ...
    for idx in top_idx:
        ...
        try:
            ...
            if f:
                ...
                if ef and et:
                    try:
                        ef2, et2 = float(ef), float(et)
                        if et2 != 0:
                            df.at[idx, "EPS Mom"] = (ef2 - et2) / abs(et2) * 100
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    m12 = pd.to_numeric(df["Mom 12M %"], errors="coerce")
    m1  = pd.to_numeric(df["Mom 1M %"],  errors="coerce")
    m6  = pd.to_numeric(df["Mom 6M %"],  errors="coerce")
    m1w = pd.to_numeric(df["Mom 1W %"],  errors="coerce")
    r_m12_1 = rank100(m12 - m1)
    r_m6_1w = rank100(m6 - m1w)
...
def show_screener(df, title="", exchange_code=""):
    ...
    st.caption(f"✅ **{len(df)}** stocks · EODHD real data (15 min delay) · Value & Growth scores ranked per country")
    ...
    if ticker_options:
        sel_ticker = st.selectbox(
            ...
            format_func=lambda t: (
                f"{t}  —  {fdf[fdf['Ticker']==t]['Company'].values[0]}"
                if not fdf[fdf["Ticker"]==t].empty else t
            ),
        ...
        if not sel_row.empty:
            ...
            vs = r.get("Value Score")
            gs = r.get("Growth Score")
            ...
            period_sel = st.radio("Chart period", list(period_map.keys()),
                                  horizontal=True, key=f"cp_{title[:10]}")
...
if page == "🏠 Dashboard":
    ...
    if not all_df.empty:
        ...
        st.markdown('<div class="section-hdr">🚀 Top 10 — Best 12-Month Momentum</div>', unsafe_allow_html=True)
        ...
        sec_raw = enriched.copy()
        sec_raw["_mc"]  = pd.to_numeric(sec_raw["Market Cap €B"], errors="coerce")
        sec_raw["_1d"]  = pd.to_numeric(sec_raw["1D %"],          errors="coerce")
        sec_raw["_12m"] = pd.to_numeric(sec_raw["Mom 12M %"],     errors="coerce")
        ...
        if sector_rows:
            ...
            sec_sel_dd = st.selectbox("Select sector to explore",
                                      sorted(sec_raw["Sector"].dropna().unique()))
            sec_stocks = sec_raw[sec_raw["Sector"] == sec_sel_dd].copy()
...
# ── EUROZONE SCREEN ───────────────────────────────────────────────
    ...
    padding:12px 20px;margin-bottom:16px;display:flex;align-items:center;gap:12px;">
        ...
        <div>
            ...
            <div style="font-size:10px;color:#5a6880;">All listed European equities · Value & Growth scored per country</div>
        </div>
    </div>""", unsafe_allow_html=True)

    st.warning("⏳ **Full enrichment of all stocks across all markets may take 10–20 minutes.** Data is cached for 30 minutes — subsequent visits are instant.")

    with st.spinner("Loading all Eurozone stocks…"):
        frames_ez = []
        for code in EXCHANGES:
            df_ex = build_exchange_df(code)
            if not df_ex.empty:
                df_ex = enrich_df(df_ex)
...
# ── SINGLE EXCHANGE SCREENS ───────────────────────────────────────
    ...
    padding:12px 20px;margin-bottom:16px;display:flex;align-items:center;gap:12px;">
        ...
        <div>
            ...
            <div style="font-size:10px;color:#5a6880;">Real data EODHD · 15 min delay · All stocks enriched with fundamentals & momentum</div>
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    if df_exch.empty:
        ...
        with st.spinner(f"Enriching all {len(df_exch)} stocks with fundamentals & momentum…"):
            df_exch = enrich_df(df_exch)
    ...
    if not pf_data:
        ...
        with st.spinner("Loading portfolio prices…"):
            ...
            for full_ticker, h in pf_data.items():
                ...
                try:
                    chg_p = float(chg_p) if chg_p is not None else 0.0
                except Exception:
                    chg_p = 0.0
                try:
                    px = float(px)
...
**Growth Score (1–100):** Ranked per country. Inputs: EPS growth, Revenue growth, EPS momentum (fwd vs trailing), 12M minus 1M momentum, 6M minus 1W momentum. Each ranked 1–100 → sum → re-ranked 1–100.
        """)
    ...
    st.dataframe(pd.DataFrame([
        ...
        ["Momentum (1W / 1M / 6M / 12M)",         "EODHD Historical EOD", "$20/mo", "Up to 30 years daily"],
