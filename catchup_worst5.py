...
def commit_log(text, path="catchup_worst5_output.txt"):
    ...
    payload = {"message": "catchup worst5 output", "content": content_b64}
...
for exchange in ["TSX", "BR", "TSE", "SEHK", "ASX"]:
    ...
    while True:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/prices_eod", headers=headers_r,
            params={"select":"ticker","exchange":f"eq.{exchange}","date":"gte.2026-07-09","limit":"1000","offset":str(offset)})
        batch = r.json()
        if not isinstance(batch,list) or not batch: break
