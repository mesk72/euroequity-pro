...
def commit_log(text, path="recompute_scores_tsx_output.txt"):
    ...
    payload = {"message": "recompute scores eu output", "content": content_b64}
...
# Il rank e' calcolato per singola borsa (country basis), come da regola
# stabilita (evita il mismatch tra mercati con norme di valutazione diverse)
grand_ok = grand_fail = 0
for exchange in TSX_EXCHANGES:
    universe_tickers = set()
    offset = 0
    while True:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/stocks", headers=headers_r,
            params={"select":"ticker","exchange":f"eq.{exchange}","in_universe":"eq.true","limit":"1000","offset":str(offset)}, timeout=30)
        batch = r.json()
    ...
    while True:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/fundamentals", headers=headers_r,
            params={"select":"ticker,exchange,pe_trailing,pe_forward,pb,eps_growth,rev_growth,mom1w,mom1m,mom6m,mom12m",
                    "exchange":f"eq.{exchange}","limit":"1000","offset":str(offset)}, timeout=30)
        data = r.json()
    ...
    if not all_data:
        log(f"{exchange}: nessun titolo in universo, salto")
        continue
    log(f"{exchange}: universo {len(universe_tickers)}, fundamentals filtrati {len(all_data)}")
    ...
    for i in range(0, len(results), 200):
        ...
        if resp.status_code in (200, 201, 204):
            ...
            fail += len(batch)
            log(f"  WARN batch {exchange}: HTTP {resp.status_code} {resp.text[:200]}")
    log(f"  {exchange}: scritti ok={ok} fail={fail}")
    grand_ok += ok; grand_fail += fail

log(f"\nTOTALE EU: ok={grand_ok} fail={grand_fail}")
commit_log("\n".join(log_lines))
print("Fatto")
