...
while True:
    r = requests.get(f"{SUPABASE_URL}/rest/v1/fundamentals", headers=headers_r,
        params={"select":"ticker,pe_trailing","exchange":"eq.OB","limit":"1000","offset":str(offset)})
    batch = r.json()
    if not isinstance(batch,list) or not batch: break
    for d in batch:
        if d["ticker"] in universe and d.get("pe_trailing") is not None and d["pe_trailing"] > 0:
            pe_values.append(d["pe_trailing"])
    offset += 1000
    if len(batch) < 1000: break

pe_values.sort()
print(f"Titoli con PE LTM positivo valido: {len(pe_values)}")

target = 14.0
below = sum(1 for v in pe_values if v < target)
rank_pct = round(below / len(pe_values) * 100)
print(f"\nUn PE LTM di {target} sarebbe piu' basso di {below}/{len(pe_values)} titoli")
