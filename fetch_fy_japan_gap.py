...
while True:
    ...
    tse_universe.update(s["ticker"] for s in batch)
    offset += 1000
...
have = set(row["ticker"] for row in reader if row["exchange"] == "TSE" and row["fiscal_month"] not in ("0",""))
missing = sorted(tse_universe - have)
print(f"Giappone da recuperare: {len(missing)}: {missing}")
...
for ticker in missing:
    ...
    try:
        ...
        if ts:
            ...
            rows_buffer.append(f"{ticker},TSE,{month}")
            ok += 1
        else:
            fail += 1
    except Exception:
